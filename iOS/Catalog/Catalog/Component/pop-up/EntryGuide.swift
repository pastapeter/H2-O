...
struct EntryGuide: View {
  ...
  private let horizonalPadding = CGFloat((UIScreen.main.bounds.width - 310) / 2).scaledWidth
  var body: some View {
    DimmedZStack {
      VStack(spacing: 20) {
        Spacer()
        ...
        HStack {
          ...
          CLPopUp(   paddingEdge: .bottom,
                     ...
                     cancelAction: { intent.send(action: .showAlertChanged(showAlert: false)) },
